...
local preview_cmds = require("advanced_git_search.commands.preview")
...
M.git_diff_commit_file_previewer = function(bufnr)
    return previewers.new_termopen_previewer({
        ...
        get_command = function(entry)
            ...
            return preview_cmds.git_diff_file(prev_commit, commit_hash, bufnr)
        end,
    })
end
...
M.git_diff_content_previewer = function()
    return previewers.new_termopen_previewer({
        ...
        get_command = function(entry)
            ...
            local prev_commit = git_utils.previous_commit_hash(commit_hash)
            return preview_cmds.git_diff_content(
                prev_commit,
                commit_hash,
                prompt
            )
...
M.changed_files_on_current_branch_previewer = function()
    return previewers.new_termopen_previewer({
        ...
        get_command = function(entry)
            return preview_cmds.git_diff_base_branch(entry.value)
        end,
    })
end
...
M.git_diff_branch_file_previewer = function(bufnr)
    ...
    return previewers.new_termopen_previewer({
        ...
        get_command = function(entry)
            ...
            return preview_cmds.git_diff_branch(branch, bufnr)
        end,
    })
end
